...
def process_content(content):
    if isinstance(content, str):
        for k, v in URLS.items():
            ...
            content = content.replace(k, v) # Fallback for just the filename
        return content
...
def deploy_full_stack():
    print("🚀 Desplegando stack completo (Ph 5-8) con URLs REALES...")
    ...
    # 4. Final Processing
    final_data = process_content(sections)

    # 5. Inject
    url = f"{SITE}/wp-json/wp/v2/pages/{PAGE_ID}"
    resp = requests.post(url, auth=HTTPBasicAuth(USER, PASS), json={
        "meta": {
            "_elementor_data": json.dumps(final_data),
            "_elementor_edit_mode": "builder"
        }
    })
    
    if resp.status_code == 200:
        print(f"✅ Homepage actualizada con éxito hasta Fase 8 utilizando URLs REALES.")
        # Cache refresh
        requests.post(url, auth=HTTPBasicAuth(USER, PASS), json={"title": "Nueva Home - Propuesta Diseño (Final F8)"})
